...
# Removes: Generator + Configuration + Service + Generated Files + Themes
...
#############################################
# PART 8: Clean Cava Config (optional)
#############################################

section "Part 8: Clean Cava Config (Optional)"

CAVA_CONFIG="$ACTUAL_HOME/.config/cava/config"
CAVA_BACKUP_DIR="$ACTUAL_HOME/.config/omarchy-themes/cava-backup"

if [ -f "$CAVA_CONFIG" ]; then
    # Check if it's an Omarcava-generated file
    if grep -q "Autogenerated by omarchy-theme-watcher" "$CAVA_CONFIG" 2>/dev/null; then
        read -p "Remove Omarcava config from Cava? [y/N]: " remove_cava_config

        if [[ "$remove_cava_config" =~ ^[Yy]$ ]]; then
            # Create backup before removing
            info "Creating backup of Cava config..."
            mkdir -p "$CAVA_BACKUP_DIR"
            cp "$CAVA_CONFIG" "$CAVA_BACKUP_DIR/config.backup"
            info "Backup saved to: $CAVA_BACKUP_DIR/config.backup"

            rm -f "$CAVA_CONFIG"
            success "Cava config removed"

            # Send reload signal to running Cava instances
            if command -v pkill &> /dev/null; then
                pkill -HUP cava 2>/dev/null && info "Reloaded running Cava instances" || true
            fi
        else
            info "Cava config kept"
        fi
    else
        info "Cava config exists but was not generated by Omarcava (skipping)"
    fi
else
    info "Cava config not found"
fi

#############################################
# PART 9: Clean tclock Wrapper (optional)
#############################################

section "Part 9: Clean tclock Wrapper (Optional)"

TCLOCK_WRAPPER="$ACTUAL_HOME/.local/bin/omarclock"

if [ -f "$TCLOCK_WRAPPER" ]; then
    # Check if it's an Omarclock-generated wrapper
    if grep -q "Autogenerated by omarchy-theme-watcher" "$TCLOCK_WRAPPER" 2>/dev/null; then
        read -p "Remove Omarclock wrapper? [y/N]: " remove_tclock_wrapper

        if [[ "$remove_tclock_wrapper" =~ ^[Yy]$ ]]; then
            rm -f "$TCLOCK_WRAPPER"
            success "Omarclock wrapper removed"
        else
            info "Omarclock wrapper kept"
        fi
    else
        info "Omarclock wrapper exists but was not generated by Omarchy (skipping)"
    fi
else
    info "Omarclock wrapper not found"
fi

#############################################
# PART 10: Clean VS Code Theme (optional)
#############################################

section "Part 10: Clean VS Code Theme (Optional)"

VSCODE_THEME_DIR="$ACTUAL_HOME/.vscode/extensions/local.theme-omarvscode"

if [ -d "$VSCODE_THEME_DIR" ]; then
    read -p "Remove Omarvscode theme from VS Code? [y/N]: " remove_vscode_theme

    if [[ "$remove_vscode_theme" =~ ^[Yy]$ ]]; then
        rm -rf "$VSCODE_THEME_DIR"
        success "VS Code theme removed"
        info "Note: Reload VS Code to apply changes"
    else
        info "VS Code theme kept"
    fi
else
    info "VS Code theme not found"
fi
...
command -v cava &> /dev/null && echo "  ${BLUE}•${NC} Cava"
command -v tclock &> /dev/null && echo "  ${BLUE}•${NC} tclock"
command -v code &> /dev/null && echo "  ${BLUE}•${NC} VS Code"
